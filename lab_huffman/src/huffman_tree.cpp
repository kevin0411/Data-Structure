                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(singleQueue.empty()){
        HuffmanTree::TreeNode* ans = mergeQueue.front();
        mergeQueue.pop();
        return ans;
    }
    else if(mergeQueue.empty()){
        HuffmanTree::TreeNode* ans = singleQueue.front();
        singleQueue.pop();
        return ans;
    }
    else{
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
    if(frequencies.empty())root_ = NULL;
    ...
    while(singleQueue.size()+mergeQueue.size()!=1){
        ...
        if(smaller != NULL && bigger != NULL){
            merge = new TreeNode(smaller->freq.getFrequency() + bigger->freq.getFrequency());
        }
        else if(bigger == NULL){
             merge = new TreeNode(smaller->freq.getFrequency());
        }
        else{
            merge = new TreeNode(0);
        }
        merge->left = smaller;
        merge->right = bigger;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.hasBits()) {
        if(bfile.getNextBit()==1){
            return new TreeNode(Frequency((char)bfile.getNextByte(), 0));
        }
        else{
            TreeNode* c = new TreeNode(0);
            c->left = readTree(bfile);
            c->right = readTree(bfile);
            return c;
        }
     }
