...
// template <class K, class V>
// void BTree<K, V>::printTree()
// {
//     /* Make the root node if the tree is empty. */
//     if (root == nullptr || root->elements.size()==0) {
//         return;
//     }
//     if(root!=NULL){
//         for(int i=0;i<(int)(root->elements.size());i++){
//             std::cout << root->elements[i].key << "/" << root->elements[i].value << " ";
//         }
//         std::cout << std::endl;
//     }
// }
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    // insert middle element to its parent
    parent->elements.insert(elem_itr, *mid_elem_itr);
    // insert new right node for children
    parent->children.insert(child_itr, new_right);

    // assign the right half of the children elements to new right
    new_right->elements.assign(mid_elem_itr+1, child->elements.end());
    // assign the right half of the children's children to new right
    new_right->children.assign(mid_child_itr, child->children.end());
    // assign the left half of the children elements to new left
    new_left->elements.assign(child->elements.begin(), mid_elem_itr);
    // assign the left half of the children's children to new left
