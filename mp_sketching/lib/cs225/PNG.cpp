...
namespace cs225 {
  void PNG::_copy(PNG const & other) {
    ...
    imageData_ = new BWPixel[width_ * height_];
  ...
  PNG::PNG(unsigned int width, unsigned int height) {
    width_ = width;
    height_ = height;
    imageData_ = new BWPixel[width * height];
  }
  ...
  bool PNG::operator== (PNG const & other) const {
    ...
    for (unsigned i = 0; i < width_ * height_; i++) {
      BWPixel & p1 = imageData_[i];
      BWPixel & p2 = other.imageData_[i];
  ...
  BWPixel & PNG::_getPixelHelper(unsigned int x, unsigned int y) const {
  ...
  BWPixel & PNG::getPixel(unsigned int x, unsigned int y) { return _getPixelHelper(x,y); }

  const BWPixel & PNG::getPixel(unsigned int x, unsigned int y) const { return _getPixelHelper(x,y); }
  ...
  bool PNG::readFromFile(string const & fileName) {
    ...
    for (unsigned i = 0; i < byteData.size(); i += 4) {
      ...
      bwColor bw = rgb2bw(rgb);
      BWPixel & pixel = imageData_[i/4];
      pixel.l = bw.l;
    }
  ...
  void PNG::resize(unsigned int newWidth, unsigned int newHeight) {
    ...
    BWPixel * newImageData = new BWPixel[newWidth * newHeight];
    ...
    for (unsigned x = 0; x < newWidth; x++) {
      for (unsigned y = 0; y < newHeight; y++) {
        if (x < width_ && y < height_) {
          BWPixel & oldPixel = this->getPixel(x, y);
          BWPixel & newPixel = newImageData[ (x + (y * newWidth)) ];
          newPixel = oldPixel;
        }
      }
    }
  ...
  std::ostream & operator << ( std::ostream& os, PNG const& png ) {
    ...
    for (unsigned x = 0; x < png.width(); x++) {
      for (unsigned y = 0; y < png.height(); y++) {
        hash ^= hashFunction(png.getPixel(x, y).l);
      }
    }
